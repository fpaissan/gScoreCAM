...
class BaseCAM:
    def __init__(
        ...
        **kwargs,
        ...
        self.activations_and_grads = ActivationsAndGradients(
            self.model, target_layers, reshape_transform=None
        )  #! set reshape_transform to None, because we will use the reshape_transform of inside BaseCAM to aviod unexpected function call of reshape_transform
    ...
    def get_cam_image(
        ...
        self.weights = self.get_cam_weights(
            input_tensor, target_layer, targets, activations, grads
        )  # usually (n, c), but with einsum, now is support any shapes that match the corresponding dimension of activation. i.e., (c, w, h)
        # weighted_activations = weights[:, :, None, None] * activations
        weight_dim = self.dim_mapper(activations.shape, self.weights.shape)
        weighted_activations = np.einsum(
            f"{weight_dim},ncwh->ncwh",
    ...
    def forward(
        ...
        if self.is_clip:
            ...
            if self.compute_input_gradient:
                input_tensor = (
                    torch.autograd.Variable(input_tensor[i], requires_grad=True)
    ...
    def compute_cam_per_layer(
        ...
        activations_list = [
            (
                self.reshape_transform(a).numpy()
        ...
        if self.is_transformer:  # use the gradient of CLS token for gradient
            ...
            grads_list = [
                (
                    self.reshape_transform(g).numpy()
                    if self.reshape_transform is not None
                    else g.numpy()
                )
                for g in self.activations_and_grads.gradients
        ...
        for i in range(len(self.target_layers)):
            ...
            cam = np.maximum(
                cam, 0
            )  # * The difference between having this line is small, i.e., in COCO val, with this line is 20.83% and without this line is 20.86%
            # normalize and resize cam
            cam = np.array([normalize2D(img) for img in cam])
            if (
                target_size is not None
            ):  # * no resize such that we can post process it later
