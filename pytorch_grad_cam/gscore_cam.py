...
class GScoreCAM(BaseCAM):
    ...
    def get_cam_weights(
        ...
        with torch.no_grad():
            ...
            upsampled = (upsampled - mins) / (
                maxs - mins + 1e-6
            )  # NOTE this could lead to division by zero

            input_tensors = (
                img_tensor[:, None, :, :].cpu() * upsampled[:, :, None, :, :]
            )
            ...
            for category, tensor in zip(target_category, top_tensors):
                # for i in tqdm.tqdm(
                # range(0, tensor.size(0), BATCH_SIZE), disable=self.mute
                # ):
                # batch = tensor[i : i + BATCH_SIZE, :]
                batch = tensor
                if self.is_clip:
                    outputs = (
                        self.model(batch.cuda(), text_tensor.cuda())[0]
                        .cpu()
                        .numpy()[:, category]
                    )
                else:
                    outputs = self.model(batch.cuda()).cpu().numpy()
                    outputs = category(outputs)
                    # outputs = outputs[:, category]
            ...
            if scores.isnan().any():
                scores = scores.nan_to_num(
                    nan=0.0
                )  # fix nan bug in clip implementation
